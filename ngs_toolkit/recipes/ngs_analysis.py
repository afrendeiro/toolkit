...
def main():
    parser = ArgumentParser(
        prog="ngs_analysis_recipe", description="NGS analysis recipe."
    )
    ...
    for data_type in data_types:
        ...
        if hasattr(prj, "sample_attributes"):
            print(
                "Using sample attributes from project configuration file: '{}'.".format(
                    ",".join(prj.sample_attributes)
                )
            )
        ...
        if "comparison_table" in prj.metadata.keys():
            ...
            print(
                "Using comparison table specified in project configuration file: '{}'.".format(
            ...
            print(
                "Will try using comparison table in metadata directory: '{}'.".format(
                    comparison_table_file
                )
            )
...
def main_analysis_pipeline(
    ...
    if data_type in ["ATAC-seq", "ChIP-seq"]:
        ...
        if data_type == "ChIP-seq":
            ...
            if comps.empty:
                print(
                    "Comparison table has no comparisons with 'data_type'=='ChIP-seq' and 'comparison_type'=='peaks'."
                )
                print("Aborting.")
                return 1
        ...
        if os.path.exists(
            os.path.join("data", "external", "HAP1_12_segments.annotated.bed")
        ):
        ...
        attrs = (
            ...
            if "sample_name" not in sample_attributes
    ...
    if data_type == "RNA-seq":
        ...
        analysis.get_gene_expression(
            samples=analysis.samples, sample_attributes=sample_attributes
        )
    ...
    if not (comps["sample_name"].value_counts() > 1).any():
        print(
            "Detected simple design for differential analysis. Each sample belongs only to one group."
        )
        analysis.differential_results = differential_analysis(
            ...
            data_type=data_type,
            samples=[
                s for s in analysis.samples if s.name in comps["sample_name"].tolist()
            ],
            covariates=None,
            alpha=0.05,
            overwrite=True,
        ...
        print(
            "Complex design for differential analysis. There are sample(s) belonging to more than one group."
        )
        ...
        for comparison in comps["comparison_name"].unique():
            ...
            analysis.differential_results = analysis.differential_results.append(
        ...
        analysis.differential_results.to_csv(
            os.path.join(
                ...
                "differential_analysis_{}".format(data_type),
    ...
    if diff.empty:
        print(
            "Differential analysis contains no significant {} at alpha {} and absolute fold change {}.".format(
                feature_name, alpha, abs_fold_change
            )
        )
        return 0
    ...
    if diff.groupby("comparison_name").count().shape[0] > 1:
        differential_overlap(
            diff, getattr(analysis, matrix).shape[0], data_type=data_type
        )
    ...
    if data_type == "RNA-seq":
        enrichment_table = pd.read_csv(
            os.path.join(
                "{}/differential_analysis_{}".format(analysis.results_dir, data_type),
        ...
        for enrichment_name, enrichment_type in [
            ...
            try:
                enrichment_table = pd.read_csv(
                    os.path.join(
                        "{}/differential_analysis_{}".format(
                            analysis.results_dir, data_type
                        ),
                        "differential_analysis" + ".{}.csv".format(enrichment_type),
                    )
                )
